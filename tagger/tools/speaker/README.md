...
混合录音优先走 MOSS diarize。MOSS 默认关闭；启用后默认使用本地
OpenMOSS Python 环境，不需要启动 API 服务。环境按上游仓库配置：

```bash
git clone https://github.com/OpenMOSS/MOSS-Transcribe-Diarize.git \
  models/MOSS-Transcribe-Diarize
uv venv --python 3.12 .runtime/moss_transcribe_diarize_py312
source .runtime/moss_transcribe_diarize_py312/bin/activate
cd models/MOSS-Transcribe-Diarize
...
`--moss-diarize-endpoint`、`--moss-diarize-timeout-sec` 和
`--moss-diarize-api-key` 只作为旧的 OpenAI-compatible HTTP 服务兼容路径保留。
只要配置了 `--moss-diarize-python` 或 `local_config.py:MOSS_DIARIZE_PYTHON`，
pipeline 就会走本地 subprocess。
