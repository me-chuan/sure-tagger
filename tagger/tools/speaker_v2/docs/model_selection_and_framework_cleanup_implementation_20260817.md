...
> 范围：Phase 0–3 的工程接入状态和 2026-08-20 直接发布行为；后续数据报告继续用于质量观测。
...
- 默认 profile 仍是 `legacy-shadow`，没有切换到 `quality-shadow`。
- `quality-shadow` 和 `lean-shadow` 只能通过 `--profile` 显式选择。
...
| Phase 4：默认切换与精简 | 部分完成 | public adapter 已直接输出；默认 profile 切换和物理精简仍未执行 |
...
- 版本化 policy schema、policy version 和稳定 SHA256 hash。
...
| Profile | 默认启用 | 默认关闭 |
| --- | --- | --- |
| `legacy-shadow` | MOSS、FireRedVAD、CAM++、Whisper Base、Sortformer、pyannote | ECAPA、Brouhaha |
| `quality-shadow` | MOSS、FireRedVAD、Sortformer、pyannote、ECAPA、Brouhaha | CAM++、Whisper Base |
| `lean-shadow` | Sortformer、pyannote、ECAPA、Brouhaha | MOSS、FireRedVAD、CAM++、Whisper Base |

### 3.2 CLI 和运行配置

`scripts/run_speaker_evidence_v2.py` 已增加：

- `--profile`，choices 直接来自 `available_profiles()`，默认值为 `legacy-shadow`。
- 八路模型的 tri-state `--*-enable` / `--*-disable`；原有 disable 参数继续兼容。
...
其中 run manifest 还会额外记录八路模型的 profile 默认状态、实际启用状态、配置和禁用原因；sample artifact 不复制整份模型 inventory。

旧调用会按 `legacy-shadow` 补齐 policy 元数据，从而保留兼容性；公开 speaker adapter 仍保持 shadow 行为，不发布 speaker 值。
...
- 接入 pipeline、CLI、profile、worker slot、missing evidence 和八模型 run manifest。

`quality-shadow` 同时保留 Brouhaha 和 FireRedVAD，`lean-shadow` 只启用 Brouhaha；默认 `legacy-shadow` 仍只启用 FireRedVAD。当前没有删除或覆盖 FireRedVAD，Brouhaha 也不会直接产生 C/M/O/X/I 投票。
...
- fusion、certification 和 run manifest 的 profile/policy version/hash 一致；公开 speaker 字段全部保持 `null`。
...
- run manifest 的八模型清单、policy 和禁用原因；
...
建议后续顺序为：补齐分层 smoke → predicted-region I 评测 → 1k 三 profile fusion/ablation → 非 AMI 评测 → license/成本/统计观测 → 按结果决定是否切换默认 profile。

## 10. 删除与回滚边界

本次没有执行任何物理清理。以下对象均保留：

- MOSS、Sortformer、pyannote、Whisper、CAM++、FireRedVAD、ECAPA、Brouhaha 的代码和模型资产；
...
当前默认仍保持 `legacy-shadow`，但这只是默认模型组合，不再是发布门禁。若显式运行 `quality-shadow` 或 `lean-shadow` 出现质量、许可或运行时问题，可切回 `--profile legacy-shadow`，不需要删除新 adapter 或评测 artifact。
