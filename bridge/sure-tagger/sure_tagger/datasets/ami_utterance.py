...
from sure_tagger.datasets.ami import meeting_type
...
def record_from_utterance(raw, source_path, line_no):
    ...
    sample_id = "AMI:%s:%s" % (audio_id, utt_id)
    ...
    return {
        ...
        "sample": {
            ...
            "native_metadata": {
                ...
                "meeting_type": meeting_type(audio_id),
                "word_count_source": len(words),
                "words": words,
            },
            "provenance": {
        ...
        with open(path, "r", encoding="utf-8") as source:
            for line_no, line in enumerate(source, 1):
                ...
                try:
                    ...
                    start_sec = record["sample"]["audio"].get("start_sec")
                    end_sec = record["sample"]["audio"].get("end_sec")
                    if start_sec is None or end_sec is None:
                        stats["duration_missing_count"] += 1
                    stats["total_text_words"] += len(text.split())
                    seen_meetings.add(record["sample"]["native_metadata"].get("meeting_id", ""))
                    seen_speakers.add(record["sample"]["native_metadata"].get("speaker_id", ""))
